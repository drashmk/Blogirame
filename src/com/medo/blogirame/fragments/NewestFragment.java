...
//		mList.setDisableScrollingWhileRefreshing(true);
		mList.setPullLabel(getString(R.string.refresh_push_down), Mode.PULL_DOWN_TO_REFRESH);
		mList.setPullLabel(getString(R.string.refresh_pull_up), Mode.PULL_UP_TO_REFRESH);
		...
		mList.setOnRefreshListener(new OnRefreshListener() {
			...
			public void onRefresh() {
				if(mList.getCurrentMode() == Mode.PULL_DOWN_TO_REFRESH) {
					executeNewServerTask(String.format(Constants.API_MAIN_NEWEST, mCurrentPage.toString()),true);
				}
				else {
					mCurrentPage++;
					String urlParam = null;
					//there has to be a way of knowing if we are viewing main page or blogs/categoires
					String queryString = getSherlockActivity().getSupportActionBar().getTitle().toString();
					//if home is displayed as up
					if(!queryString.equalsIgnoreCase(getString(R.string.app_name))) {
						//get the category slug by its name 
						DataBaseCategories database = new DataBaseCategories(getSherlockActivity());
						String serverString = database.getSlugByName(queryString);
						database.closeDB();
						urlParam = String.format(Constants.API_CATEGORIES_NEWEST, serverString, mCurrentPage.toString());
					}
					else {
						//we are on main page
						urlParam = String.format(Constants.API_MAIN_NEWEST, mCurrentPage.toString());
					}
		...
		tempList.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view, int pos, long id) {
				//the list has a header that counts as a view so we need to get the items at positon - 1
		...
		tempList.setOnScrollListener(new AbsListView.OnScrollListener() {
			...
			public void onScroll(AbsListView view, int firstVisibleItem, int visibleItemCount, int totalItemCount) {
				...
				//but when the user reaches the top of the page than bring it back on
		...
		mTask.execute(String.format(Constants.API_MAIN_NEWEST, mCurrentPage.toString()));
		//initialize the ad
	...
	public void executeNewServerTask(String urlParam, boolean shouldResetPageCounter) {
	...
	private class GetFromServer extends AsyncTask<String, Integer, ArrayList<ListObjectBlogPost>> {
			...
			try {
				...
				//if not, proceed with making the adapter
						...
						for(int i=0; i<jsonArray.length(); i++) {
							...
							tempObject.setmDate(jsonBlogPost.getString(Constants.JSON_PARAM_DATE));
		...
		protected void onPostExecute(ArrayList<ListObjectBlogPost> result) {
			if(result == null) {
				...
				//if we have more results they should be appended to the originals
				if(mCurrentPage != 0) {
					//get the existing array
					ListView tempList = mList.getRefreshableView();
					ListAdapterNewest apendedAdapter = new ListAdapterNewest(getSherlockActivity(), mAllResults, getSherlockActivity());
					tempList.setAdapter(apendedAdapter);
					//set selection to the last 20 items so the user dosn't scroll down 
					...
					ListView tempList = mList.getRefreshableView();
					ListAdapterNewest adapter = new ListAdapterNewest(getSherlockActivity(), result, getSherlockActivity());
				...
				getSherlockActivity().findViewById(R.id.img_splash).setVisibility(View.GONE);
				//load fade in animation
				getSherlockActivity().findViewById(R.id.linear_main).setAnimation(AnimationUtils.loadAnimation(getSherlockActivity(), R.anim.fade_in));
			}
			super.onPostExecute(result);
		}
	}
	
	private class GetAdsFromServer extends AsyncTask<String, Integer, ArrayList<String>> {
		...
		protected ArrayList<String> doInBackground(String... params) {
			try {
				...
				if(jsonObject.getBoolean(Constants.JSON_PARAM_ADS_HAS) == false)
					return toReturn;
				toReturn.add(jsonObject.getString(Constants.JSON_PARAM_ADS_TITLE));
				toReturn.add(jsonObject.getString(Constants.JSON_PARAM_ADS_DESCRIPTION));
		...
		protected void onPostExecute(final ArrayList<String> result) {
			...
			if(result.size() == 0)
				...
				//get the whole ad
				final RelativeLayout ad = (RelativeLayout) mView.findViewById(R.id.relative_newest_ads);
				
				//get the add title and description
				TextView adTitle = (TextView) mView.findViewById(R.id.txt_newest_ad_title);
				TextView adDescription = (TextView) mView.findViewById(R.id.txt_newest_ad_description);
				//set them accordingly 
				adTitle.setText(result.get(0));
				adDescription.setText(result.get(1));
				
				//get the close button
				ImageButton adClose = (ImageButton) mView.findViewById(R.id.img_newest_ad_close);
				adClose.setOnClickListener(new View.OnClickListener() {
					
					@Override
					public void onClick(View v) {
						ad.setAnimation(AnimationUtils.loadAnimation(getActivity(), R.anim.slide_out_to_bottom));
						ad.setVisibility(View.INVISIBLE);
					}
				});
				
				//get the linear layout representing the ad 
				LinearLayout adLayout = (LinearLayout) mView.findViewById(R.id.linear_newest_ad);
				...
				adLayout.setOnClickListener(new View.OnClickListener() {
					...
					public void onClick(View v) {
						...
						Intent browserIntent = new Intent(Intent.ACTION_VIEW, Uri.parse(result.get(2)));
						startActivity(browserIntent);
					}
				});
				
				//show the ad
				ad.setAnimation(AnimationUtils.loadAnimation(getActivity(), R.anim.slide_in_from_bottom));
